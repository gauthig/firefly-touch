 ...
 * pure-C codec. The scan step is the only genuinely new piece.
 */
...
static esp_ble_scan_params_t s_scan_params = {
    .scan_type          = BLE_SCAN_TYPE_ACTIVE,   /* need scan responses for the name */
...
static void start_scan(void)
{
    s_state = WD_SCANNING;
    s_buf_len = 0;
    esp_err_t err = esp_ble_gap_start_scanning(WD_SCAN_DURATION_S);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "start_scanning failed: %s", esp_err_to_name(err));
    }
}
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *p)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        char buf[32];
        const size_t n = name_len < sizeof(buf) - 1 ? name_len : sizeof(buf) - 1;
        memcpy(buf, name, n);
        buf[n] = '\0';

        if (!hughes_wd_name_matches(buf)) {
            break;
        }

        memcpy(s_bda, p->scan_rst.bda, sizeof(esp_bd_addr_t));
        s_bda_type = p->scan_rst.ble_addr_type;
        s_bda_known = true;
        snprintf(s_peer_str, sizeof(s_peer_str), "%02X:%02X:%02X:%02X:%02X:%02X",
                 s_bda[0], s_bda[1], s_bda[2], s_bda[3], s_bda[4], s_bda[5]);
        ESP_LOGI(TAG, "found '%s' at %s (addr type %d)", buf, s_peer_str, s_bda_type);
        esp_ble_gap_stop_scanning();
        break;
    }

    case ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT:
        if (s_bda_known && s_state == WD_SCANNING) {
            try_connect();
        }
        break;
